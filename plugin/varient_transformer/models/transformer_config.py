...
@dataclass
class VarientEncDecBaseConfig(FairseqDataclass):
    ...
    #jc
    talking_heads: Optional[str] = field(
        default=None, metadata={"help": "apply talking heads in self-attention"}
    )
    


@dataclass
class VarientDecoderConfig(VarientEncDecBaseConfig):
...
@dataclass
class VarientTransformerConfig(FairseqDataclass):
    ...
    encoder: VarientEncDecBaseConfig = VarientEncDecBaseConfig()
    ...
    decoder: VarientDecoderConfig = VarientDecoderConfig()
            ...
            for fld in fields(cls):
                ...
                if fld.name == "decoder":
                    if safe_hasattr(args, "decoder"):
                        ...
                        seen.add("decoder")
                        config.decoder = VarientDecoderConfig(**args.decoder)
                    else:
                        config.decoder = cls._copy_keys(
                            args, VarientDecoderConfig, "decoder", seen
                        )
                elif fld.name == "encoder":
                    # same but for encoder
                    if safe_hasattr(args, "encoder"):
                        seen.add("encoder")
                        config.encoder = VarientEncDecBaseConfig(**args.encoder)
                    else:
                        config.encoder = cls._copy_keys(
                            args, VarientEncDecBaseConfig, "encoder", seen
                        )
